...
class PortForImpAndExp extends Component {
    state = {
        data:[]
    }
    componentDidMount() {
        this.getGoodsStatisticsByPortServer();
    }
    getGoodsStatisticsByPortServer() { 
        server.getGoodsStatisticsByPort({ terminalId: this.props.portAreaId, year: this.props.year }, (data) => { 
            if (data) { 
                this.getGoodsStatisticsByPortData(data)
            }
        })
    }
    getGoodsStatisticsByPortData(data) { 
        let newData = [];
...
export default PortForImpAndExp;